...
export default function KycPage() {
  ...
  return <AppLayout title="KYC reviews" subTitle="Review submitted identity documents and selfies securely">
    ...
    <Modal opened={opened} onClose={close} title="KYC review" size="xl" centered>
      {selected && <Stack gap="lg"><Group><Avatar src={selected.user.avatar} name={selected.user.fullname} size={52} /><Stack gap={0}><Text fw={700}>{selected.user.fullname}</Text><Anchor href={`/user-management/${selected.userId}`}>{selected.user.email}</Anchor><Text fz="xs" c="var(--fj-text-muted)">{selected.user.phoneNumber}</Text></Stack></Group><Text fz="sm" c="var(--fj-text-muted)">Document: {selected.documentType || "—"} · Country: {selected.countryIso || "—"} · Number: {selected.documentNumber || "—"}</Text><Divider />{part("identity", selected.identity.status, "Identity document", selected.evidence.documentFrontUrl)}{selected.evidence.documentBackUrl && <Image src={selected.evidence.documentBackUrl} alt="Back of identity document" radius="sm" maw={360} fit="contain" />}{part("selfie", selected.selfie.status, "Selfie", selected.evidence.selfieUrl)}{selected.history.length > 0 && <><Divider /><Stack gap="sm"><Text fw={700}>Previous submissions</Text><Text fz="xs" c="var(--fj-text-muted)">Archived evidence is read-only and is kept for review and compliance.</Text>{selected.history.map((item) => <Stack key={item.id} gap="xs" p="sm" style={{ border: "1px solid var(--fj-border-subtle)", borderRadius: 10 }}><Group justify="space-between"><Text fw={600}>{item.submissionType === "identity" ? "Identity document" : "Selfie"}</Text><Badge color={color[item.status]} variant="light">{item.status}</Badge></Group><Text fz="sm">{item.documentType || "Selfie"}{item.countryIso ? ` · ${item.countryIso}` : ""}{item.documentNumber ? ` · ${item.documentNumber}` : ""}</Text>{item.reason && <Text fz="sm" c="red">Decline reason: {item.reason}</Text>}{item.evidence.documentFrontUrl && <Image src={item.evidence.documentFrontUrl} alt="Archived identity document" radius="sm" maw={300} fit="contain" />}{item.evidence.documentBackUrl && <Image src={item.evidence.documentBackUrl} alt="Archived identity document back" radius="sm" maw={300} fit="contain" />}{item.evidence.selfieUrl && <Image src={item.evidence.selfieUrl} alt="Archived selfie" radius="sm" maw={300} fit="contain" />}</Stack>)}</Stack></>}</Stack>}
    </Modal>
  </AppLayout>;
}
