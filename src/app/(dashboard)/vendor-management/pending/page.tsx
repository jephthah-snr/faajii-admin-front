...
const PendingVendors = () => {
  ...
  const vendorsData = vendors?.data?.inactiveVendors || [];
  const inactiveVendorsCount = vendors?.data?.pagination?.inactive?.total || 0;
  ...
  const rows = vendorsData?.map((data) => {
    return (
      <Table.Tr
        key={data?.id}
        onClick={() => router.push(`/vendor-management/pending/${data?.id}`)}
    ...
    <AppLayout title="Vendors" hasBackButton>
      <Flex direction="column" gap={20}>
        ...
        <PpTable
          ...
          rowData={rows}
          totalItems={inactiveVendorsCount}
