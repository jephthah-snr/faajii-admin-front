...
const FaajiiStore = ({
  ...
  const [inventoryItem, setInventoryItem] = useState<Store | null>(null);
  const [
    inventoryModalOpened,
    { open: openInventoryModal, close: closeInventoryModal },
  ] = useDisclosure(false);
  const [
  ...
  const handleUpdateInventory = (item: Store) => {
      ...
      <Table.Td>
        <Menu shadow="md">
          ...
          <Menu.Dropdown onClick={(event) => event.stopPropagation()}>
            <Menu.Item onClick={() => handleUpdateInventory(item)}>
              Update Inventory
            </Menu.Item>
            <Menu.Divider />
            <Menu.Item
